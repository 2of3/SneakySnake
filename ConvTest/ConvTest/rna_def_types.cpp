...
std::map <std::string, rna_def_type> rna_def_map_str;
std::map <rna_def_type, std::string> rna_def_map_enum;

void setup_enum_mapping()
{
	for (int en_itr = rna_def_none; en_itr != rna_def_world; en_itr++)
	{
		rna_def_type en_item = static_cast<rna_def_type>(en_itr);
		rna_def_map_str[rna_def_name[en_itr]] = en_item;
		rna_def_map_enum[en_item] = rna_def_name[en_itr];
	}

	// change unnecessary types
	std::string tmpstr = get_rnadef_str(rna_def_function_flag);
	rna_def_map_str[tmpstr] = rna_def_none;
}

std::string extract_rnadef_type(std::string str)
{
...
rna_def_type get_rnadef_enum(std::string str)
{
	str = extract_rnadef_type(str);

	if (rna_def_map_str.find(str) != rna_def_map_str.end())
		return rna_def_map_str[str];

	return rna_def_unknown;
}

std::string get_rnadef_str(rna_def_type val)
{
	if (rna_def_map_enum.find(val) == rna_def_map_enum.end())
		return rna_def_map_enum[val];
